def generate_prompt(extracted_text, include_details):
    ...
    5. Retain the original currency format for prices (e.g., '$', '₹', 'rupees'). If the price field contains an 'S' character mistakenly converted from '$', replace the 'S' with '$'. If no price is mentioned, set the price field to `null`. Strictly ensure that the price field is a valid 'integer', 'number', or 'float'.
    ...
    10. Ensure that every food item and category are separated using commas so that the structure is clear and parsable.
